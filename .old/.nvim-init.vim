...
let g:dashboard_custom_section={
\ 'history_list': {
    \ 'description': [' History                 LDR f h'],
    \ 'command': ':History<CR>' },
\ 'buffer_list': {
    \ 'description': ['﬘ Buffer                  LDR f b'],
    \ 'command': ':Buffers<CR>' },
\ 'find_files': {
    \ 'description': [' Files                   LDR f f'],
    \ 'command': ':Files<CR>' },
\ 'explorer': {
    \ 'description': [' Browse                  LDR e e'],
    \ 'command': ':Vifm<CR>' },
\ 'fuzzy': {
    \ 'description': [' Fuzzy                   LDR f a'],
    \ 'command': ':Rg' }
\ }
" header {{{
" let g:dashboard_custom_header = [
    ...
    \]
" }}}
let g:total_plugins = len(keys(g:plugs))
...
lua <<EOF
require'nvim-treesitter.configs'.setup {
   ensure_installed= {"python","html","cpp","css","vim","latex"},
        highlight = {
        enable = true,
